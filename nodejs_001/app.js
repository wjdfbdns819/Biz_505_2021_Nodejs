/**
 * nodejs 프로젝트에서 많은 외부 모듈을 import하여 사용하게 된다
 *  import 된 모듈을 직접 호출하여 기능을 구현하거나
 *  또는 다른 모듈과 협력하여 background에서 작동되기도 한다
 *
 * 이처럼 모듈을 import 하면 프로젝트에서는 이들을 middie ware가
 *  설정(치)되었다 라고 한다
 */
...
//  repsones하는 역할을 수행하는 middle ware 이며, framwork이다
...
// localhost:3000/ 라고 요청이 오면
//  ./routes 폴더에 있는 index.js 파일을 실행하여 요청을 처리하라
// Spring에서
//  클래스에 설정된 @RequestMapping에 해당하는 부분임
var indexRouter = require("./routes/index");

// localhost:3000/users 라고 요청이 오면
//  ./routes 폴더에 있는 users.js 파일을 실행하여 요청을 처리하라
var usersRouter = require("./routes/users");

//home 뒤에 붙는 .js는 생략이 가능함
...
// 수행해야 하는데, path.join() 함수를 사용하여 쉽게 지정할 수 있다
// 지금 현재는 운영체제에 무관하게 path(폴더 경로)를
// 슬래시(/)를 사용하지만
// 과거에는 윈도우에서는 역슬래시(\)를 사용하고,
// Unix(linux, mac)등에서는 슬래시(/) 사용을 하여야만 했던 시절이 있다
//  이때부터 path와 같은 도구를 사용하여 코드가 실행되는 환경에 적합하도록 만들어서 사용했다
