...
// fecth를 사용하여 서버에 데이터를 요청하기 위해 별도의 함수를 선언하기
...
document.addEventListener("DOMContentLoaded",()=>{
	...
	// table id 값을 추출하기 위하여 article.order_list에서 dataset을 추출하여 변수에 담기
	...
	// product_article이 없는 page에서 script 오류가 나는 것을 방지 하기 위하여
	if(product_article) {
		product_article.addEventListener("click", (e) => {
			...
			if(target.tagName === "DIV" && target.className.includes("menu")) {
				...
				//document.location.href = `/pos/order/${menu_id}`;
